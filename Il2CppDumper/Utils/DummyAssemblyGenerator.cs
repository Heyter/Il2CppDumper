...
public class DummyAssemblyGenerator
{
    ...
    public DummyAssemblyGenerator(Il2CppExecutor il2CppExecutor, bool addToken)
    {
        ...
        foreach (var imageDef in metadata.imageDefs)
        {
            var typeEnd = imageDef.typeStart + (int)imageDef.typeCount;
        ...
        foreach (var imageDef in metadata.imageDefs)
        {
            ...
            for (var index = imageDef.typeStart; index < typeEnd; ++index)
            {
                ...
                if (addToken)
                {
                    var customTokenAttribute =
                        new CustomAttribute(typeDefinition.Module.ImportReference(tokenAttribute));
                    customTokenAttribute.Fields.Add(
                        new CustomAttributeNamedArgument(
                            "Token",
                            new CustomAttributeArgument(stringType, $"0x{typeDef.token:X}")));
            ...
            for (int index = imageDef.typeStart; index < typeEnd; index++)
            {
                ...
                for (var i = typeDef.fieldStart; i < fieldEnd; ++i)
                {
                    ...
                    if (addToken)
                    {
                        ...
                        customTokenAttribute.Fields.Add(
                            new CustomAttributeNamedArgument(
                                "Token",
                                new CustomAttributeArgument(stringType, $"0x{fieldDef.token:X}")));
                    ...
                    if (metadata.GetFieldDefaultValueFromIndex(i, out var fieldDefault) && fieldDefault.dataIndex != -1)
                    {
                        ...
                        else
                        {
                            var customAttribute = new CustomAttribute(
                                typeDefinition.Module.ImportReference(metadataOffsetAttribute));
                            var offset = new CustomAttributeNamedArgument(
                                "Offset",
                                new CustomAttributeArgument(stringType, $"0x{value:X}"));
                    ...
                    {
                        var fieldOffset = il2Cpp.GetFieldOffsetFromIndex(
                            index,
                        ...
                        {
                            var customAttribute = new CustomAttribute(
                                typeDefinition.Module.ImportReference(fieldOffsetAttribute));
                            var offset = new CustomAttributeNamedArgument(
                                "Offset",
                                new CustomAttributeArgument(stringType, $"0x{fieldOffset:X}"));
                            customAttribute.Fields.Add(offset);
                            fieldDefinition.CustomAttributes.Add(customAttribute);
                        }
                    }
                ...
                for (var i = typeDef.methodStart; i < methodEnd; ++i)
                {
                    ...
                    if (addToken)
                    {
                        ...
                        customTokenAttribute.Fields.Add(
                            new CustomAttributeNamedArgument(
                                "Token",
                                new CustomAttributeArgument(stringType, $"0x{methodDef.token:X}")));
                    ...
                    for (var j = 0; j < methodDef.parameterCount; ++j)
                    {
                        ...
                        var parameterDefinition = new ParameterDefinition(
                            parameterName,
                            (ParameterAttributes)parameterType.attrs,
                            parameterTypeRef);
                        methodDefinition.Parameters.Add(parameterDefinition);
                        parameterDefinitionDic.Add(methodDef.parameterStart + j, parameterDefinition);
                        //ParameterDefault
                        if (metadata.GetParameterDefaultValueFromIndex(
                                methodDef.parameterStart + j,
                                out var parameterDefault) && parameterDefault.dataIndex != -1)
                        {
                            if (executor.TryGetDefaultValue(
                                    parameterDefault.typeIndex,
                    ...
                    if (!methodDefinition.IsAbstract)
                    {
                        ...
                        {
                            var customAttribute =
                                new CustomAttribute(typeDefinition.Module.ImportReference(addressAttribute));
                            var fixedMethodPointer = il2Cpp.GetRVA(methodPointer);
                            var rva = new CustomAttributeNamedArgument(
                                "RVA",
                                new CustomAttributeArgument(stringType, $"0x{fixedMethodPointer:X}"));
                            var offset = new CustomAttributeNamedArgument(
                                "Offset",
                                new CustomAttributeArgument(stringType, $"0x{il2Cpp.MapVATR(methodPointer):X}"));
                            var va = new CustomAttributeNamedArgument(
                                "VA",
                                new CustomAttributeArgument(stringType, $"0x{methodPointer:X}"));
                            customAttribute.Fields.Add(rva);
                            customAttribute.Fields.Add(offset);
                            customAttribute.Fields.Add(va);
                            if (methodDef.slot != ushort.MaxValue)
                            {
                                var slot = new CustomAttributeNamedArgument(
                                    "Slot",
                                    new CustomAttributeArgument(stringType, methodDef.slot.ToString()));
                ...
                for (var i = typeDef.propertyStart; i < propertyEnd; ++i)
                {
                    ...
                    var propertyDefinition =
                        new PropertyDefinition(propertyName, (PropertyAttributes)propertyDef.attrs, propertyType)
                        {
                    ...
                    if (addToken)
                    {
                        ...
                        customTokenAttribute.Fields.Add(
                            new CustomAttributeNamedArgument(
                                "Token",
                                new CustomAttributeArgument(stringType, $"0x{propertyDef.token:X}")));
                ...
                for (var i = typeDef.eventStart; i < eventEnd; ++i)
                {
                    ...
                    var eventDefinition = new EventDefinition(
                        eventName,
                        (EventAttributes)eventType.attrs,
                        eventTypeRef);
                    if (eventDef.add >= 0)
                    {
                    ...
                    if (addToken)
                    {
                        ...
                        customTokenAttribute.Fields.Add(
                            new CustomAttributeNamedArgument(
                                "Token",
                                new CustomAttributeArgument(stringType, $"0x{eventDef.token:X}")));
            ...
            foreach (var imageDef in metadata.imageDefs)
            {
                ...
                for (int index = imageDef.typeStart; index < typeEnd; index++)
                {
                    ...
                    for (var i = typeDef.methodStart; i < methodEnd; ++i)
                    {
                        ...
                        CreateCustomAttribute(
                            imageDef,
                            methodDef.customAttributeIndex,
                            methodDef.token,
                            typeDefinition.Module,
                        ...
                        for (var j = 0; j < methodDef.parameterCount; ++j)
                        {
                            ...
                            CreateCustomAttribute(
                                imageDef,
                                parameterDef.customAttributeIndex,
                                parameterDef.token,
                                typeDefinition.Module,
                    ...
                    for (var i = typeDef.propertyStart; i < propertyEnd; ++i)
                    {
                        ...
                        CreateCustomAttribute(
                            imageDef,
                            propertyDef.customAttributeIndex,
                            propertyDef.token,
                            typeDefinition.Module,
                            propertyDefinition.CustomAttributes);
                    }
            ...
            case Il2CppTypeEnum.IL2CPP_TYPE_VAR:
                {
                    if (memberReference is MethodDefinition methodDefinition)
                    {
                        return CreateGenericParameter(
                            executor.GetGenericParameterFromIl2CppType(il2CppType),
    ...
    private void CreateCustomAttribute(
        Il2CppImageDefinition imageDef,
        ...
        if (attributeIndex >= 0)
        {
            try
            {
                if (il2Cpp.Version < 29)
                {
                    ...
                    for (var i = 0; i < attributeTypeRange.count; i++)
                    {
                        ...
                        if (!TryRestoreCustomAttribute(typeDefinition, moduleDefinition, customAttributes))
                        {
                            ...
                            var fixedMethodPointer = il2Cpp.GetRVA(methodPointer);
                            var customAttribute =
                                new CustomAttribute(moduleDefinition.ImportReference(attributeAttribute));
                            var name = new CustomAttributeNamedArgument(
                                "Name",
                                new CustomAttributeArgument(stringType, typeDefinition.Name));
                            var rva = new CustomAttributeNamedArgument(
                                "RVA",
                                new CustomAttributeArgument(stringType, $"0x{fixedMethodPointer:X}"));
                            var offset = new CustomAttributeNamedArgument(
                ...
                else
                {
                    ...
                    var attributeDataOffset = metadata.Version >= 38
                        ? metadata.header.attributeData.offset
                        : metadata.header.attributeDataOffset;
                    ...
                    if (reader.Count != 0)
                    {
                        for (var i = 0; i < reader.Count; i++)
                        {
                            ...
                            foreach (var argument in visitor.Arguments)
                            {
                                ...
                                var customAttributeArgument = CreateCustomAttributeArgument(
                            ...
                            foreach (var field in visitor.Fields)
                            {
                                ...
                                var customAttributeArgument = CreateCustomAttributeArgument(
                                    ...
                                    field.Value,
                                    fieldDefinition);
                                var customAttributeNamedArgument = new CustomAttributeNamedArgument(
    ...
    private static bool TryRestoreCustomAttribute(
        ...
        IGenericParameterProvider iGenericParameterProvider)
    {
        if (!genericParameterDic.TryGetValue(param, out var genericParameter))
        {
            ...
            for (var i = 0; i < param.constraintsCount; ++i)
            {
                ...
                genericParameter.Constraints.Add(
                    new GenericParameterConstraint(
                        GetTypeReference((MemberReference)iGenericParameterProvider, il2CppType)));
            }
        }
    ...
    private CustomAttributeArgument CreateCustomAttributeArgument(
        ...
        MemberReference memberReference)
    {
        ...
        if (typeReference.FullName == "System.Object")
        {
            ...
            {
                val = new CustomAttributeArgument(
                    memberReference.Module.ImportReference(typeof(Type)),
