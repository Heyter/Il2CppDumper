...
public class Il2CppDecompiler
{
    ...
    public void Decompile(Config config, string outputDir)
    {
        ...
        for (var imageIndex = 0; imageIndex < metadata.imageDefs.Length; imageIndex++)
        {
            ...
            writer.Write(
                $"// Image {imageIndex}: {metadata.GetStringFromIndex(imageDef.nameIndex)} - {imageDef.typeStart}\n");
        }
        ...
        foreach (var imageDef in metadata.imageDefs)
        {
            try
            {
                ...
                for (int typeDefIndex = imageDef.typeStart; typeDefIndex < typeEnd; typeDefIndex++)
                {
                    ...
                    else if ((typeDef.flags & TYPE_ATTRIBUTE_INTERFACE) == 0 &&
                             (typeDef.flags & TYPE_ATTRIBUTE_ABSTRACT) != 0)
                    {
                        writer.Write("abstract ");
                    }
                    else if (!typeDef.bitfield.ValueType && !typeDef.bitfield.EnumType &&
                    ...
                    if (config.DumpField && typeDef.field_count > 0)
                    {
                        ...
                        for (var i = typeDef.fieldStart; i < fieldEnd; ++i)
                        {
                            ...
                            {
                                writer.Write(
                                    GetCustomAttribute(imageDef, fieldDef.customAttributeIndex, fieldDef.token, "\t"));
                            }
                            ...
                            writer.Write(
                                $"{executor.GetTypeName(fieldType, false, false)} {metadata.GetStringFromIndex(fieldDef.nameIndex)}");
                            if (metadata.GetFieldDefaultValueFromIndex(i, out var fieldDefaultValue) &&
                                fieldDefaultValue.dataIndex != -1)
                            {
                                if (executor.TryGetDefaultValue(
                                        fieldDefaultValue.typeIndex,
                                        fieldDefaultValue.dataIndex,
                                        out var value))
                                {
                                    writer.Write(" = ");
                    ...
                    if (config.DumpProperty && typeDef.property_count > 0)
                    {
                        ...
                        for (var i = typeDef.propertyStart; i < propertyEnd; ++i)
                        {
                            ...
                            if (propertyDef.get >= 0)
                            {
                                ...
                                writer.Write(
                                    $"{executor.GetTypeName(propertyType, false, false)} {metadata.GetStringFromIndex(propertyDef.nameIndex)} {{ ");
                            }
                        ...
                        for (var i = typeDef.methodStart; i < methodEnd; ++i)
                        {
                            ...
                            if (config.DumpMethodOffset)
                            {
                                ...
                                if (!isAbstract && methodPointer > 0)
                                {
                                    ...
                                    writer.Write(
                                        "\t// RVA: 0x{0:X} Offset: 0x{1:X} VA: 0x{2:X}",
                                        fixedMethodPointer,
                            ...
                            for (var j = 0; j < methodDef.parameterCount; ++j)
                            {
                                ...
                                if (parameterType.byref == 1)
                                {
                                    if ((parameterType.attrs & PARAM_ATTRIBUTE_OUT) != 0 &&
                                        (parameterType.attrs & PARAM_ATTRIBUTE_IN) == 0)
                                    {
                                        parameterStr += "out ";
                                    }
                                    else if ((parameterType.attrs & PARAM_ATTRIBUTE_OUT) == 0 &&
                                             (parameterType.attrs & PARAM_ATTRIBUTE_IN) != 0)
                                    {
                                        parameterStr += "in ";
                                    }
                            ...
                            if (il2Cpp.methodDefinitionMethodSpecs.TryGetValue(i, out var methodSpecs))
                            {
                                ...
                                foreach (var group in groups)
                                {
                                    ...
                                    if (genericMethodPointer > 0)
                                    {
                                        ...
                                        writer.Write(
                                            $"\t|-RVA: 0x{fixedPointer:X} Offset: 0x{il2Cpp.MapVATR(genericMethodPointer):X} VA: 0x{genericMethodPointer:X}\n");
                                    }
                                    else
                                    {
                                        writer.Write("\t|-RVA: -1 Offset: -1\n");
                                    ...
                                    foreach (var methodSpec in group)
                                    {
                                        var (methodSpecTypeName, methodSpecMethodName) =
                                            executor.GetMethodSpecName(methodSpec);
    ...
    {
        if (methodModifiers.TryGetValue(methodDef, out var str))
        {
            return str;
        }
