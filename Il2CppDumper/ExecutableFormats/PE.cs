...
public sealed class PE : Il2Cpp
{
    ...
    public override ulong MapVATR(ulong absAddr)
    {
        ...
        var section =
            sections.FirstOrDefault(x => addr >= x.VirtualAddress && addr <= x.VirtualAddress + x.VirtualSize);
        if (section == null)
        {
            return 0ul;
        }
    ...
    {
        var section =
            sections.FirstOrDefault(x => addr >= x.PointerToRawData && addr <= x.PointerToRawData + x.SizeOfRawData);
        if (section == null)
        {
            return 0ul;
        }
